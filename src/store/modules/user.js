...
const state = {
  //i use local storage, because i persisted the localstorage, so the user will be able to refresh the
  //page. if you do not want to persist the localstorage, just uncomment the state at the bottom and comment away
  //this state.
...
const actions = {
  ...
  [CHANGEOWNPASSWORD]: ({ commit, dispatch }, formData) => {
    return new Promise((resolve, reject) => {
      commit(UPDATE_ONE_USER);
      apiCall({
        url: api_routes.user.update_one + store.getters.userId,
        data: formData,
        method: "put"
      })
        .then(resp => {
  ...
  [RESETUSERPASSWORD]: ({ commit, dispatch }, id) => {
    return new Promise((resolve, reject) => {
      apiCall({
        url: api_routes.user.change_password + "/" + id,
        method: "put"
      })
        .then(resp => {
